...
from copy_claude.core.config import McpServerConfig
from copy_claude.core.mcp.client import McpClient
from copy_claude.core.mcp.tool import McpTool
...
# 依次连接每个 MCP server，发现工具后缓存供后续 registry 使用；失败时记录日志并跳过
    async def start_all(self, servers: list[McpServerConfig]) -> None:
        for cfg in servers:
            try:
                client = await self._connect(cfg) # 基于配置连接返回client:如果是stdio就启动本地紫禁城，如果是tcp就从cfg获取ip端口
                tool_defs = await client.list_tools()
                for tool_def in tool_defs: # 客户端的工具添加到mcp server
