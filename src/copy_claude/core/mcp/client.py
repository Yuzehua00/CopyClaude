...
class McpClient:
    ...
    async def connect_stdio(
        ...
        self._proc = await asyncio.create_subprocess_exec( # 子程序执行mcp_server的程序，然后与之通信即可。
            command, *args,
    ...
    async def _drain_stderr(self) -> None: # 一直循环读取报错的管道。
        if self._proc is None or self._proc.stderr is None:
            return
        try:
            while True:
                line = await self._proc.stderr.readline()
                # 会阻塞直到读取到换行符或 EOF。即使 stderr 没有数据，await 会让出事件循环，不会浪费 CPU
                if not line: # 如果读取到空字节串（b''），表示子进程的 stderr 已到达 EOF（进程关闭了 stderr 或已退出），此时退出循环，任务结束。
                    break
    ...
    async def _call(self, method: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        async with self._lock:
            await self._write_line(json.dumps(request)) # 向mcp server发出请求，那边必然接收到，
            while True:
    ...
    async def _write_line(self, line: str) -> None:
        data = (line + "\n").encode()
        if self._transport == "stdio": # 传输格式stdio就用self._proc 疑惑，在哪里与mcpserver建立连接了呢？
            w = self._proc.stdin if self._proc else None # self.proc.stdin是StreamWriter，用于发送信息给mcpserver
            if w is None:
