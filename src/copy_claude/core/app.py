...
from copy_claude.core.session.manager import SessionManager
...
class CoreApp:  # 所有用户的命令处理器都在这里定义，真正的核心业务逻辑。
    ...
    async def _agent_run_handler(self, params: Dict[str:Any]) -> AgentRunResult:  # 在守护进程启动AgentRunner。
        ...
        session = await self._sessions.create("one_shot",title=cmd.goal[:40])
        run_id = new_run_id()
        run_task = asyncio.create_task(self._sessions.send_message(session.id,cmd.goal,run_id)) # 创建任务
    ...
    async def _session_create_handler(self, params: Dict[str:Any]) -> SessionCreateResult: # 创建会话命令
        assert self._sessions is not None
    ...
    async def _session_send_message_handler(self, params: Dict[str:Any]) -> SessionSendMessageResult:
        assert self._sessions is not None
    ...
    async def _session_history_handler(self, params: Dict[str:Any]) -> SessionGetHistoryResult:
        ...
        messages = self._sessions.get_history(cmd.session_id)
