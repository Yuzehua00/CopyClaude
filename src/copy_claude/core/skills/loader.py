...
class SkillLoader:
    _BUILTIN_DIR = Path(__file__).parent / "builtin" # 这是skills/builtin下存放技能的地方
    ...
    def _search_paths(self, name: str) -> list[Path]:
        dirs = [
            Path(".copyclaude/skills"), # 项目级技能
            Path("~/.copyclaude/skills").expanduser(), # C盘里的全局技能
            self._BUILTIN_DIR, # src内部自带的
        ]
        paths: list[Path] = []
        for d in dirs:
